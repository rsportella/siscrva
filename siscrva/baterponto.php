...
<html lang="pt-br">
    ...
    <?php
    $ultimoPonto = $_PS['link']->prepare("SELECT `data`, `hora` FROM `bateponto` WHERE `funcionario` = " . $_SESSION['funcionarioID'] . " ORDER BY `idbatePonto` DESC LIMIT 1");
    $ultimoPonto->execute();
...
//verifica o ultimo ponto do funcionario mais 15min é menor ou igual a data/hora no momento
    ...
    if (date('Y-m-d H:i:s') >= date('Y-m-d H:i:s', strtotime('+15 minute', strtotime($dataHora)))) {
        $baterPonto = $_PS['link']->prepare("INSERT INTO `bateponto` (`funcionario`, `data`, `hora`) VALUES ('" . $_SESSION['funcionarioID'] . "', '" . date('Y-m-d') . "', '" . date('H:i:s') . "')");
        $baterPonto->execute();
    ...
    <body class="alert alert-<?php echo $msg['tipo'] ?> text-center" role="alert">
        </br></br> <img src="img/ctpa/<?php echo $_SESSION['funcionarioCPF'] ?>.jpg" class="img-thumbnail" width="200" /></br></br></br>
